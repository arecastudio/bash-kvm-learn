...
#qemu-img create -b jammy-server-cloudimg-amd64.img -f qcow2 -F qcow2 ubuntu9000.img 20G
qemu-img create -b jammy-server-cloudimg-amd64.img -f qcow2 -F qcow2 $name.img 20G

#genisoimage -output cidata.iso -V cidata -r -J user-data meta-data

#virt-install --name=ubuntu9000 --ram=1024 --vcpus=1 --import --disk path=ubuntu9000.img,format=qcow2 --disk path=cidata.iso,device=cdrom --os-variant=ubuntu22.04 --network bridge=virbr0,model=virtio --graphics vnc,listen=0.0.0.0 --noautoconsole
